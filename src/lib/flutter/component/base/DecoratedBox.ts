...
import { Constraint, Size, Radius, BorderStyle, Offset } from "$lib/flutter/type"
import type { Border } from "$lib/flutter/type/_types/borderstyle"
...
class RenderDocoratedBox extends SingleChildRenderObject {
  ...
  protected override performPaint(context: PaintContext, offset: Offset): void {
    ...
    rectEl.setAttribute('width', `${this.size.width - (borderLeft.thickness + borderRight.thickness) / 2}`)
    rectEl.setAttribute('height', `${this.size.height - (borderBottom.thickness + borderTop.thickness) / 2}`)
  ...
  private assertEqualBorder(border1: Border, border2: Border) {
    if(border1.color !== border2.color || border2.thickness !== border1.thickness) {
      throw {message: 'sorry! It is not implemented to set different border style'}
    }
  }
}
