...
class RenderDocoratedBox extends SingleChildRenderObject {
	...
	protected override preformLayout(): void {
		...
		if (this.child != null) {
			this.child.layout(
				new Constraint({
					...this.constraint,
					maxHeight: this.constraint.maxHeight - (top.thickness + bottom.thickness),
					maxWidth: this.constraint.maxWidth - (left.thickness + right.thickness)
				})
			);
	...
	protected performPaint({ rect: rectEl }: { [key: string]: SVGElement }): void {
		const {
			color,
			border: { top: borderTop, left: borderLeft, right: borderRight, bottom: borderBottom },
			radius: {
