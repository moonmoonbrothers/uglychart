...
export type MainAxisAlignment = 'start' | 'end' | 'spaceEvenly' | 'spaceBetween' | 'spaceAround';
...
class RenderFlex extends MultiChildRenderObject {
	...
	protected preformLayout(): void {
		...
		const flexUnitSize = (mainAxisValue - childIntrinsicMainAxisValue) / totalFlex;
		...
		this.children.forEach((child, i) => {
			...
			child.offset[crossAixsOffset] = this.getChildOffsetOnCrossAixs(child.size[this.crossAxis]);
		});
	}
	...
	private getChildOffsetsOnMainAxis(childMainAxisValues: number[]) {
		...
		const restSpaceSize = this.size[this.mainAxis] - childMainAxisValues.reduce(sum, 0);
	...
	override getIntrinsicHeight(): number {
		...
		const childInstrinsicHeights = this.children.map((child) => child.getIntrinsicHeight());
	...
	override getIntrinsicWidth(): number {
		const sum = (acc: number, value: number) => acc + value;
		const max = (acc: number, value: number) => Math.max(acc, value);
		const childInstrinsicWidths = this.children.map((child) => child.getIntrinsicWidth());
