...
class RenderBaseGrid extends MultiChildRenderObject {
    ...
    */
	...
	private get rows(): GridTemplate[] {
		return Array.from({ length: this.rowCount }, (_, i) => this.templateRows[i] ?? this.autoRow);
	}
	...
	protected preformLayout(): void {
		...
		const contentFitColumnWidths: number[] = Array.from({ length: this.columnCount }, () => 0);
		const contentFitRowHeights: number[] = Array.from({ length: this.rowCount }, () => 0);
		...
		const fixedColumnWidths: number[] = this.columns.map(({ type, value }, i) => {
			let result = 0;
			if (type === 'content-fit') {
		...
		const frUnitOnHieght = (this.size.height - totalFixedHeight) / totalFrOnHeight;
...
export class GridTemplate {
	...
	value: number;
	constructor({ type, value }: { value: number; type: 'px' | 'percent' | 'fr' | 'content-fit' }) {
		this.type = type;
		this.value = value;
	}
